...
# Copy of Сравнение неофициальной интеграции WhatsApp и официальной WhatsApp Business API с Битрикс24
...
* **Прозрачная стоимость:** фиксированный платеж в размере **3600 рублей в месяц (за 30 дней)** за одну подключенную Открытую линию (WhatsApp-приложение).
...
Официальный WhatsApp Business API позволяет интегрировать мессенджер с CRM-системой, сайтом и другими инструментами. Прямой доступ к API (программному интерфейсу) предоставляет компания Meta (Facebook), и им обладают лишь несколько крупных провайдеров в мире. Наше приложение **Олчат WABA** выполняет роль связующего звена, позволяя подключить ваш WhatsApp к Битрикс24 через одного из таких провайдеров — компанию **GupShup**.
...
* **Нельзя использовать стандартные приложения:** официальный API (программный интерфейс) несовместим с приложениями WhatsApp, WhatsApp Business и веб-версией ([web.whatsapp.com](https://web.whatsapp.com/)).
* Номер можно сменить, но процесс привязки имеет **важные особенности**. После подключения телефонного номера к WhatsApp Business API его нельзя сразу использовать в обычном приложении WhatsApp. \
  Перед подключением к WABA номер необходимо полностью удалить из всех личных аккаунтов WhatsApp. Как вернуть номер для WhatsApp, можно узнать [здесь](kak-otklyuchit-nomer-ot-whatsapp-business-api-chtoby-podklyuchitsya-k-seromu-whatsapp.md).

{% hint style="warning" %}
Также рекомендуем создать резервную копию чатов, чтобы избежать потери переписок, так как после удаления аккаунта восстановить данные невозможно.
{% endhint %}
...
1. Оплата за приложение для интеграции с Битрикс24 (нам, в рублях). Подробнее в статье [Оплата приложения Олчат WABA](https://waba.docs.olchat.io/tarify-prilozheniya/oplata-prilozheniya-whatsabi) и в разделе [Тарифы Олчат WABA](copy-of-sravnenie-neoficialnoi-integracii-whatsapp-i-oficialnoi-whatsapp-business-api-s-bitriks24.md#tarify-olchat-waba).
...
#### Хотите подключить Олчат WABA? Есть вопросы по официальной интеграции? Напишите нам в [чат поддержки](https://auth2.bitrix24.net/oauth/select/?preset=im\&IM_DIALOG=networkLines7c380c91ab28dacab02d3af93fecdbf9).

_\*Деятельность Meta запрещена на территории Российской Федерации._
