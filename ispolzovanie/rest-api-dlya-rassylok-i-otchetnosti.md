...
Токен выдаётся на главной странице приложения Олчат в Битрикс24 — блок «Клиентское API» под списком подключённых номеров, кнопка «Выдать / перевыпустить токен». Рядом показывается базовый адрес API.
