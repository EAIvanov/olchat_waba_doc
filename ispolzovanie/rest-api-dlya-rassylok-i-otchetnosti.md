...
Токен выдаётся в приложении Олчат — «•••» (меню вызова настроек коннектора) — Настройки коннектора — блок «Клиентское API» — «Сгенерировать токен».

Токен принадлежит порталу Битрикс24 целиком, а не отдельной линии: одним токеном вы работаете со всеми подключёнными номерами портала, указывая нужный в параметре `line_number`.

Кнопка «Сгенерировать новый» выдаёт новый токен и одновременно обесценивает предыдущий — все интеграции на старом значении сразу начнут получать ошибку `401`.
...
Лимит частоты считается отдельно для каждого метода: обращение к `messages.stats.get` не расходует лимит `campaigns.start`.
...
`campaigns.stats.get` (параметр `campaign_id`) отдаёт воронку: `total`, `pending`, `sent`, `delivered`, `read`, `replied`, `errors`, `skipped`, `no_whatsapp`, а также статус рассылки и время запуска и завершения.

`campaigns.recipients.list` (параметры `campaign_id`, `status`, `page`, `page_size`) — построчный отчёт: номер, `external_id`, статус, ошибка, время отправки, доставки, прочтения и ответа.
...
Статусы получателя: `pending` (ожидает), `processing` (в обработке), `success` (отправлено), `error` (ошибка), `skipped` (пропущен при отмене), `no_whatsapp` (на номере нет WhatsApp).
